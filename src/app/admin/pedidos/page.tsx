...
import { Package, Truck, CheckCircle, Clock, XCircle, type LucideIcon } from 'lucide-react';
...
const statusConfig: Record<string, { label: string; color: string; bg: string; icon: LucideIcon }> = {
  ...
  delivered: { label: 'Entregado', color: 'text-[var(--green)]', bg: 'bg-[var(--green-light)]', icon: CheckCircle },
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
          {Object.entries(statusConfig).map(([key, config]) => {
            ...
            return (
              <button key={key} onClick={() => setFilterStatus(filterStatus === key ? 'all' : key)} className={`p-4 text-left transition-all ${filterStatus === key ? 'ring-2 ring-[var(--green)] bg-white shadow-sm' : 'bg-white shadow-sm border border-[var(--border)]'}`}>
