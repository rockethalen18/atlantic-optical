...
export default function FeaturesStrip() {
  ...
  return (
    <section className="bg-[var(--dark-bg)] border-b border-white/5">
      <div ref={ref} className="max-w-[1680px] mx-auto px-6 md:px-10">
        <div className="grid grid-cols-2 lg:grid-cols-4">
          {features.map((f, i) => (
            <div key={i} className={`feat-card group flex items-center gap-5 py-7 px-6 md:px-8 border-white/5 ${i < 3 ? 'border-r' : ''} transition-colors hover:bg-white/[0.02]`}>
              <div className="w-12 h-12 bg-white/5 border border-white/10 flex items-center justify-center flex-shrink-0 group-hover:border-[var(--blue)]/30 transition-colors">
